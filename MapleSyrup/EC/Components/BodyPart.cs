...
public class BodyPart : IComponent
{
            ...
            case BodyType.Body:
                {
                    _matrix = Matrix.CreateTranslation(new Vector3(-Parent.Transform.Position, 0f)) *
                              ...
                              Matrix.CreateScale(new Vector3(1, 1, 0f)) *
            ...
            case BodyType.Head:
                {
                    _matrix = Matrix.CreateTranslation(new Vector3(-Parent.Transform.Position, 0f)) *
                              Matrix.CreateTranslation(new Vector3(-_origin, 0f)) *
                              Matrix.CreateRotationZ(0f) *
                              Matrix.CreateTranslation(new Vector3(GetMap(BodyMap.BodyNeck), 0f)) *
    ...
    public void AddFrame(int frame, Vector2 origin, Texture2D texture)
    {
        _frames.Add((frame, origin, texture));
    }
